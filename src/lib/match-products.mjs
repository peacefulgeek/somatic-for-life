/**
 * Product matching engine — selects 3-4 relevant products per article.
 * Uses named parameters only (never change to positional — the argument-flip bug brought down sites).
 */
...
function scoreProduct(product, title, tags, category) {
  ...
  // Boost books slightly
  if (product.type === 'book') score += 1;
